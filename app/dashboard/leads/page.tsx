...
export default function LeadsDashboardPage() {
  ...
  const [agents, setAgents] = useState<any[]>([]); // لیست کل مشاوران برای دراپ‌داون کلیک‌راست
  const [currentUser, setCurrentUser] = useState<any>(null); // پلمب گارد ادمین/ناظر
  ...
  const loadSeniorConsultants = async () => {
    ...
    try {
      ...
      const usersRes = await fetch(`${BACKEND_BASE_URL}/api/next/users`, { headers: authHeaders });
      if (usersRes.ok) {
  ...
  // 🎯 استیت‌های مودال جدید مگا-پرسونا
  ...
  // 👑 دیتابیس غنی ۱۱ پرسونای روان‌شناختی شما مجهز به منطق، روش و روزهای فالوآپ دقیق
  ...
  const loadData = () => {
    ...
    let url = `${BACKEND_BASE_URL}/api/next/dashboard/leads?page=${currentPage}&view_mode=${viewMode}&per_page=15&sort_by=${sortConfig.key}&sort_dir=${sortConfig.direction}`;
  ...
  const handleInlineUpdate = async (leadId: number, field: string, value: any) => {
    ...
    // گارد فوق امنیتی: اگر فیلد تغییر مشاور باشد و کاربر ناظر نباشد، عملیات مسدود می‌شود
  ...
  // مپ کردن کلید نمایشی پرسونا در گرید
...
{/* ================= 📊 سیستم ناوبری و پیجینیشن متمرکز نئونی ================= */}
        <div className="bg-slate-50 dark:bg-slate-900/60 px-4 py-3 flex items-center justify-between border-t select-none">
          <div className="flex flex-1 justify-between sm:hidden">
            <button
              ...
              className="relative inline-flex items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-40 cursor-pointer"
            >
              ⏮️ صفحه قبل
            </button>
            <button
              ...
              className="relative ml-3 inline-flex items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-40 cursor-pointer"
            >
              صفحه بعد ⏭️
            </button>
          </div>
          ...
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between w-full">
            ...
            <div>
              <nav className="isolate inline-flex -space-x-px rounded-xl shadow-2xs gap-1" aria-label="Pagination">
                {/* دکمه بازگشت به صفحه اول */}
                <button
                  onClick={() => setCurrentPage(1)}
                  disabled={currentPage === 1 || loading}
                  className="relative inline-flex items-center rounded-lg bg-white dark:bg-slate-800 border p-1.5 text-slate-500 hover:bg-slate-50 disabled:opacity-30 cursor-pointer transition-colors"
                  title="صفحه اول"
                >
                  «
                </button>
                ...
                <button
                  onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                  disabled={currentPage === 1 || loading}
                  className="relative inline-flex items-center rounded-lg bg-white dark:bg-slate-800 border px-2.5 py-1.5 text-[10px] font-black text-slate-600 dark:text-slate-300 hover:bg-slate-50 disabled:opacity-30 cursor-pointer transition-colors"
                >
                  قبلی
                </button>

                {/* 🧠 رندر داینامیک دکمه‌های شماره صفحات (محدوده هوشمند ۲ صفحه قبل و بعد) */}
                {Array.from({ length: lastPage }, (_, index) => index + 1)
                  .filter(page => page === 1 || page === lastPage || Math.abs(page - currentPage) <= 1)
                  .map((page, idx, array) => {
                    // قرار دادن علامت سه نقطه برای صفحات جاافتاده میانبر
                    const showEllipsis = idx > 0 && page - array[idx - 1] > 1;

                    return (
                      <React.Fragment key={page}>
                        {showEllipsis && (
                          <span className="relative inline-flex items-center px-2 py-1.5 text-[10px] font-bold text-slate-400">...</span>
                        )}
                        <button
                          type="button"
                          onClick={() => setCurrentPage(page)}
                          disabled={loading}
                          className={`relative inline-flex items-center rounded-lg px-3 py-1.5 text-[10px] font-mono font-black transition-all cursor-pointer border ${
                            currentPage === page
                              ? 'z-10 bg-indigo-600 text-white border-indigo-700 shadow-sm shadow-indigo-500/20'
          ...
          <form onSubmit={handleSaveEvent} className="bg-white p-6 rounded-[28px] border w-[420px] shadow-2xl space-y-4 text-right">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="date" lang="fa-IR" className="w-full p-2 border rounded-xl bg-slate-50 font-mono text-center font-bold outline-none" value={eventForm.session_date_shamsi} onChange={e => setEventForm({ ...eventForm, session_date_shamsi: e.target.value })} required />
              ...
              <div>
                ...
                <input type="date" lang="fa-IR" className="w-full p-2 border rounded-xl bg-slate-50 font-mono text-center font-bold outline-none" value={eventForm.next_call_date_shamsi} onChange={e => setEventForm({ ...eventForm, next_call_date_shamsi: e.target.value })} />
            ...
            <div>
              ...
              <select className="w-full p-2.5 border rounded-xl bg-slate-50 text-indigo-600 font-black outline-none cursor-pointer" value={eventForm.assigned_agent_id} onChange={e => setEventForm({ ...eventForm, assigned_agent_id: e.target.value })} required>
            ...
            <div className="flex gap-2">
              ...
              <button type="submit" className="flex-1 bg-slate-900 text-white font-black py-2 rounded-xl text-xs">🔏 ثبت دوره و ارجاع</button>

              <button type="button" onClick={() => setEventModalLeadId(null)} className="px-4 bg-slate-100 text-slate-600 font-bold py-2 rounded-xl">انصراف</button>
      ...
      {contextMenu && (() => {
        ...
        // پیدا کردن آبجکت کارشناس فعلی متقاضی از روی دیتای گرید
        ...
        return (
          ...
          >
            ...
            {/* 🎯 سوییچر دراپ‌داون تخصیص کارشناس پلمب‌شده در بدنه کلیک‌راست */}
            <div className="px-3 py-1.5">
              ...
              <select
                ...
                disabled={!isSupervisor} // 🔏 قفل سیستمی: ایجنت‌های معمولی حق تغییر ندارند
                onChange={e => {
                  handleInlineUpdate(contextMenu.lead.id, 'agent_id', e.target.value);
                  setContextMenu(null); // بستن اتوماتیک منو پس از کلیک
                }}
              >
